...
// ===============================zset=================================
     ...
     * @param scode
     */
    public boolean zadd(String key, Object value, double scode){
        try{
           return redisTemplate.opsForZSet().add(key, value, scode);
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
     ...
     * @return
     */
    public Double zincrby(String key, Object value,double scode){
        try {
            return redisTemplate.opsForZSet().incrementScore(key,value,scode);
        }catch (Exception e) {
            e.printStackTrace();
            return 0D;
        }
    ...
    /**
     * 返回指定key对应的有序集合中，索引在min~max之间的元素信息，
     *      如果带上 withscores 属性的话，可以将分值也带出来
     * @param key
     * @param min
     * @return
     */
     ...
     * @return
     */
    public Cursor<ZSetOperations.TypedTuple<Object>> zscan(String key, Long count,String pattern){
        ...
        try {
            scanOptions= ScanOptions.scanOptions()
                                    ...
                                    .match(pattern)
                                    .build();
            cursor = redisTemplate.boundZSetOps(key).scan(scanOptions);
        }catch (Exception e) {
            e.printStackTrace();
        }
